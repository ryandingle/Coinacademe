<?php
	...
	if($_GET['show_date'] != '')
	{
		$config['base_url'] = base_url().'worker/in/menu?tab=published&show_date='.$_GET['show_date'].'';
	...
	elseif($_GET['search'] != '')
	{
		$config['base_url'] = base_url().'worker/in/menu?tab=published&search='.$_GET['search'].'';
	...
	{
		$config['base_url'] = base_url().'worker/in/menu?tab=published';
		$this->db->where('author',$this->session->userdata('id'));
		$this->db->where('status','publish');
		$config['total_rows'] = $this->db->get('article')->num_rows();
		$config['per_page'] = 20;
                ...
                <tr>
                    ...
                    <div>
                        <div class="action-links" style="height: 20px;width: 100%">
                            ...
                            <a href="worker/in/menu?tab=edit_article&id=<?php echo $row->article_id;?>&status=<?php echo $row->status;?>" class="quick_action" data-item-id="<?php echo $row->article_id;?>" data-action="edit" data-selection="article">Edit</a>
                             | 
